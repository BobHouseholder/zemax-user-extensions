    ...
    class Program
    {
        ...
        static void ParseArgs(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                switch (a)
                {
                    case "-n": Opts.N = int.Parse(next(), ci); break;
                    case "-pitch": Opts.Pitch = double.Parse(next(), ci); break;
                    case "-dot": Opts.DotDia = double.Parse(next(), ci); break;
                    case "-plate": Opts.Plate = double.Parse(next(), ci); break;
                    case "-thick": Opts.PlateT = double.Parse(next(), ci); break;
                    case "-material": Opts.Material = next(); break;
                    case "-coating": Opts.Coating = next(); break;
                    case "-film": Opts.Film = double.Parse(next(), ci); break;
                    case "-drawlimit": Opts.DrawLimit = int.Parse(next(), ci); break;
                    case "-rig": Opts.Rig = true; break;
