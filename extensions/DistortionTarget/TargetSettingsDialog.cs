...
namespace DistortionTarget
{
    ...
    class TargetSettingsDialog : Form
    {
        ...
        TargetSettingsDialog(Options o)
        {
            ...
            // 470, not 430: the derived read-out is the widest thing in the dialog and
            // at 430 its longest line needed 444 px against 426 available, so it rendered
            // truncated. Width is driven by that label, not by the input rows.
            const int GW = 470, PAD = 12;
            int y = PAD;
            ...
            // Height 96, not 74: a Label with AutoSize off WRAPS rather than
            // clipping horizontally, so the width only has to be reasonable — but a
            // wrapped line needs somewhere to go. No fixed width survives arbitrary
            // input (a 1999-dot grid on a 1200 mm plate needs 486 px against 466),
            // so the read-out is allowed to run to a fifth and sixth line instead of
            // being truncated. Measured with GetPreferredSize against the real width.
            _derived = new Label
            {
                Left = PAD + 2,
                Top = y,
        ...
        TextBox Field(GroupBox g, int row, string label, string value)
        {
            ...
            var tb = new TextBox { Left = 210, Top = top, Width = 230, Text = value };
            g.Controls.Add(tb);
            return tb;
        }
        ...
        void Recompute()
        {
            ...
            _derived.Text = string.Format(CI,
                ...
                "drawing {7:n0} of {8:n0} dots — drawing only, affects no traced result\r\n" +
                "radiometry needs ray splitting on, or no coating is applied",
        ...
        static void LoadLastRun(Options o)
        {
            try
            {
                ...
                foreach (string line in File.ReadAllLines(SettingsPath))
                {
                    ...
                    // an explicit command-line flag outranks the saved last run
